...
class Comment extends React.Component {
    ...
    componentDidMount() {
        this.props.getInfo.getComments(parseInt(this.props.match.params.id), 1, 10);
    }
    ...
    createList() {
        ...
        return list.map((item, index) => {
            return (
                <li key={index}>
                    <div className="list-title"><Link to={`/user/center/${item.user.id}`}>{item.user.name}</Link> 评论了电影 <Link
