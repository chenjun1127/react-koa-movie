...
class Collect extends React.Component {
    componentDidMount() {
        this.props.getInfo.getCollects(parseInt(this.props.match.params.id));
    }
            ...
            return (
                <li key={index}>
                    <div className="list-title">
                        <Link to={`/user/center/${item.user.id}`}>{item.user.name}</Link> 收藏了电影 <Link to={`/movies/detail/${item.m.movieId}`}>{item.m.movieName}</Link>
                    </div>
